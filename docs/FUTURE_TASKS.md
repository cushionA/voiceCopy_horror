...
- [ ] 🟢 ✓ KnnVcConverter 完全 GPU 化 (TopK + IndexSelect も GPU)
  - 発生元: PR #6 レビュー (2026-05-09) 提案 #9
  - 現状: GPU で距離行列 [N1, N2] 計算後、CPU で top-K 選別 + ms 行平均 (合計 ~43ms)
  - 対応方針: `Functional.TopK` で indices [N1, K] を GPU 算出 → `Functional.IndexSelect` で ms 行 gather → `Functional.ReduceMean` で平均
  - 期待効果: readback サイズ [N1, N2]=10MB → [N1, dim]=7MB に半減、CPU 後処理 43ms → 数 ms に
  - 制約: 重みつき除外 (weight=0 frame) を GPU 経路で表現する必要 → `Where(weights > 0, dist, +inf)` 等
  - 対象: `Assets/SentisSpike/Scripts/KnnVc/KnnVcConverter.cs`
  - 優先度低の理由: 現状 65ms で十分速い、Vocode/Extract が支配的なのでこちらの最適化効果は限定的

- [ ] 🟢 ✓ MatchingSetPool に float[] キャッシュを公開 (DownloadToArray 削減)
  - 発生元: PR #6 レビュー (2026-05-09) 提案 #8
  - 現状: KnnVcConverter.Convert 毎回 `matchingSet.DownloadToArray()` で ~6MB を CPU にコピー (バッチ 40 回で 240MB GC 圧)
  - 対応方針: `MatchingSetPool` 内部に float[] バッキングを保持し、`AsReadOnlySpan` か `GetUnderlyingArray` で公開。`Append` 時のみ無効化
  - 制約: matchingSet が `Append` で grow するたびにキャッシュ再構築が必要
  - 対象: `Assets/SentisSpike/Scripts/KnnVc/MatchingSetPool.cs` + `KnnVcConverter.cs`
  - 優先度低の理由: GC 圧は大きいが現実プレイ規模 (3 秒 utterance × たまに発火) では誤差レベル、batch ストレステスト固有の問題
