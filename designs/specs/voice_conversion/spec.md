...
- **VC-007** (Phase 8 PR #8 追加): 事前トークン化された query features (`Tensor<float>` shape `[N, 1024]`)
  を入力に取る Convert overload を提供し、固定 source (少女セリフ等) の演出時 WavLM ステージを
  完全 skip できる。実測で **batch mean -34% / p95 -47%** の latency 改善を達成
  (`VcTestOutput/perf_20260510_135417/perf.csv`)。プレイヤー録音側は実時間 forward 必須のため
  WavLM モデル本体のロードは継続要
...
- **MS-003**: 2 つのプール (target / player) の **重みつき kNN** で変換ができる
  (実装: 連結プール + frame ごとの weight、kNN distance 計算時に weight でスケーリング。
  α=1.0 で純 target、α=0.0 で純 player、α=0.5 で均等混合)
- **MS-004**: プールは Unity `Application.persistentDataPath` に **npy 形式** で永続化される
  (Python サイドで生成・検証可能、Phase 9 収録時の audio→npy バッチ変換ワークフローで利用)
- **MS-005**: プール 1 件の追加 (3 秒録音) は 0.5 秒以下で完了する
- **MS-006**: target プール作成時、推奨カバレッジを満たさない場合は warning ログを出す
  (段階目標: スパイク 30秒 / MVP 3分 / 本番 10分。15000 frames = 5 分相当を「飽和ライン」)

### B-3 Speaker Similarity (BAD/GOOD 判定)

- **SS-001**: 2 つの WavLM 特徴プールに対し、コサイン類似度を **[0.0, 1.0]** 範囲で算出できる
  (生 cosine sim は [-1, 1] だが `max(0, cos)` でクランプして「類似してない」=0 に統一。
  1.0 = 同一、0.0 = 無関係 or 反対)
- **SS-002**: プレイヤー全録音 vs 少女声プールの類似度判定が、エンディング分岐の閾値判定に使える
  - 高類似 (default 仮閾値: > 0.85) → BAD ED 「カラダを奪われる」
  - 中類似 (0.40-0.70) → 「声を変えて攻略 → 電話切れ」分岐
  - 低類似 (< 0.40) → GOOD ED 「脱出成功」
  - **閾値は仮、Phase 8 で 5 名程度の内部テストプレイヤー録音で実測キャリブレーション**
- **SS-003**: 閾値は ScriptableObject で調整可能 (本番リリース後も hot-fix 可能化)
...
- **SR-002**: **タイトル画面 → ゲーム開始 (ニューゲーム / コンティニュー) の遷移時**に
  warmup forward を 1 回実行する (cold start ~510ms をシーン遷移 loading に隠す)
  - ホラーゲームの没入感を破らないため、初回 VC 必要時 (= 少女幽霊の最初の電話) ではなく
    シーン遷移で隠す方針
- **SR-003**: BackendType.GPUCompute をデフォルトとし、GPU 不在時は CPU フォールバックする
- **SR-004**: ゲーム終了時、Sentis Worker を Dispose して GPU メモリを解放する
  (`OnDestroy()` で `Dispose()` を呼ぶ)
...
## Resolved Questions (人間 spec-gate review 2026-05-09)

- **OQ-1 (target reference 品質基準)** → **段階的**: スパイク 30秒 → MVP 3分 → 本番 10分。
  声優収録スケジュール柔軟化、MVP リリース時点で 3分でも実用、Phase 10+ で 10分版に置換
- **OQ-2 (永続化フォーマット)** → **npy 形式**。NpyWriter 既存実装と対称化、Python サイドで
  生成・検証可能なメリット (Phase 9 ワークフローに最適)
- **OQ-3 (ED 閾値)** → **Phase 8 で 5 名内部キャリブレーション + ScriptableObject で hot-fix 可能化**。
  公開ベータでテレメトリ収集は Phase 11 以降に検討
- **OQ-4 (warmup タイミング)** → **タイトル → ゲーム開始遷移時**にシーン遷移 loading で隠す。
  没入感破壊を避ける

## 追加検討事項 (review 時に判明、Resolved)

- **A-1 (cosine 類似度の値域)** → `[0, 1]` クランプ (`max(0, cos)`)。「類似してない」=0 で直感的
- **A-2 (Service の lifecycle)** → MonoBehaviour + IDisposable で `OnDestroy()` で `Dispose()` 呼ぶ
- **A-3 (Architect 整合)** → `Architect/10_音声変換システム.md` を Phase 7 内 (実装着手前) に新規追加
- **A-4 (α 混合実装方式)** → **重みつき kNN**。連結プール + frame ごとの weight をスケーリング。
  プールサイズ固定、α 連続変化に強い
- **A-5 (テストデータ)** → **TestSounds を gitignore whitelist で git 追跡** (~5MB)。
  `my_sampleVoice.wav` は user 個人音声なので Steam 配信時注意 (テスト後削除推奨)
